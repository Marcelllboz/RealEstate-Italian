...
**Architecture:** Static HTML/CSS/JS (no framework, no build step) reading five JSON files from `dashboard/data/`. Daily and weekly scheduled Claude agents rewrite `brief.json`/`stats.json` and push; Vercel auto-redeploys. Italian/skill blocks rotate client-side by date. Tasks persist in localStorage.
...
**Files:**
- Create: `dashboard/data/config.json`
- Modify: `.vercelignore`

- [ ] **Step 1: Create `dashboard/data/config.json`**
...
**Files:**
- Create: `dashboard/data/brief.json`, `dashboard/data/stats.json`

- [ ] **Step 1: Create `dashboard/data/brief.json`** with realistic sample content. This exact schema is what the daily agent must produce — every key shown is required; `lines` may be `[]`; set the sample `date` to the build day so the page renders fresh during development:
...
- [ ] **Step 2: Create `dashboard/data/stats.json`** (the weekly agent's contract):
...
- [ ] **Step 3: Validate and commit** — `python3 -c "import json;[json.load(open(f)) for f in ['dashboard/data/brief.json','dashboard/data/stats.json','dashboard/data/config.json']]"` (expect no output), then `git add dashboard/data && git commit -m "feat: sample brief and stats data"`

### Task 3: Rotation banks

**Files:**
- Create: `dashboard/data/italian.json`, `dashboard/data/skills.json`

- [ ] **Step 1: Create `dashboard/data/italian.json`.** Source the content from `02-ITALIAN-PHRASE-BANK.md` and `01-SA-REAL-ESTATE-STUDY-PACK.md` (real-estate-relevant phrases first, general conversational fill after). **Minimum 90 entries.** Schema (array order is the rotation order):
...
- [ ] **Step 2: Create `dashboard/data/skills.json`.** Source from `03-SCRIPTS.md` plus standard objection/technique repertoire. **Minimum 60 entries**, schema:
...
- [ ] **Step 3: Verify counts and commit** — `python3 -c "import json;print(len(json.load(open('dashboard/data/italian.json'))['entries']), len(json.load(open('dashboard/data/skills.json'))['entries']))"` (expect `>=90 >=60`), then commit `feat: italian and skills rotation banks`.
...
**Files:**
- Modify: `dashboard/index.html` (script block)
...
```
You maintain dashboard/data/brief.json in this repo. Read dashboard/data/config.json for the suburb list.
...
```
You maintain dashboard/data/stats.json. Read dashboard/data/config.json for suburbs.
...
- [ ] Ventuno Giorni complete → promote dashboard: move root `index.html` to `ventuno.html`, copy `dashboard/index.html` route or add a root redirect — decide at the time with Marcello, then update `.vercelignore` accordingly and push.
